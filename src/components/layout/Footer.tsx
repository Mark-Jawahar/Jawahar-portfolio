...
const SvgIcon = ({ path, viewBox = "0 0 24 24" }: { path: string; viewBox?: string }) => (
  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox={viewBox} fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <path d={path} />
  </svg>
);
      ...
      <div className="max-w-[1100px] mx-auto">
        <div className="flex flex-col md:flex-row items-center justify-between gap-8 mb-10">
          <div className="flex items-center gap-4">
            {socialLinks.map((link) => (
              <a
                ...
                className="flex items-center justify-center w-10 h-10 rounded-full bg-white/[0.03] border border-white/[0.06] text-muted hover:text-pearl hover:bg-white/[0.06] hover:border-white/[0.12] transition-all duration-300"
          ...
          <button
            ...
            className="flex items-center gap-2 text-xs text-muted hover:text-ice transition-colors"
        ...
        <div className="text-center text-[0.65rem] text-muted/60 leading-relaxed">
          <p>&copy; {new Date().getFullYear()} {profile.name}. Crafted with intention.</p>
          <p className="mt-0.5">Built from scratch with Next.js, Tailwind CSS, and Framer Motion.</p>
        </div>
      </div>
    </footer>
  );
