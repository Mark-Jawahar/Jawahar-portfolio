            ...
            <ScrollReveal key={project.title} delay={0.05 * i}>
              <div className="glass-card p-5 h-full group">
                <div className="flex items-start justify-between mb-3">
                  <span className={`text-[0.55rem] font-semibold px-2 py-0.5 rounded-full border uppercase tracking-wider ${categoryColors[project.category] || "text-muted border-white/10 bg-white/[0.03]"}`}>
                    {project.category}
                  </span>
                </div>
                <h3 className="text-sm font-semibold text-pearl mb-1.5 group-hover:text-gradient-accent transition-all duration-300">{project.title}</h3>
                <p className="text-[0.7rem] text-muted leading-relaxed mb-3">{project.description}</p>
