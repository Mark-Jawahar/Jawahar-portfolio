  ...
  return (
    ...
    >
      <div className="relative rounded-[var(--radius-xl)] overflow-hidden bg-[var(--color-bg-secondary)] aspect-[3/4] shadow-[var(--shadow-xs)]">
        ...
        <div className="absolute top-4 left-4 flex flex-col gap-1.5">
          {product.isBestSeller && (
            <span className="px-3 py-1 rounded-full bg-[var(--color-accent)]/90 backdrop-blur-md text-[10px] font-medium text-white tracking-[0.08em] uppercase">
              Best Seller
            </span>
          )}
          {product.isNew && (
          ...
          {product.comparePrice && (
            <span className="px-3 py-1 rounded-full bg-[var(--color-error)]/90 text-[10px] font-medium text-white tracking-[0.08em] uppercase">
              Sale
            </span>
          )}
        </div>
        ...
        <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-all duration-500 translate-x-2 group-hover:translate-x-0">
          <button
            className="w-10 h-10 rounded-full bg-[var(--color-glass-deep)] backdrop-blur-xl border border-[var(--color-glass-border)] flex items-center justify-center hover:bg-[var(--color-text-primary)] hover:text-white transition-all duration-300 shadow-[var(--shadow-glass)]"
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-2 group-hover:translate-y-0">
          <Link
            ...
            className="block w-full text-center h-11 flex items-center justify-center rounded-[var(--radius-lg)] bg-[var(--color-glass-deep)] backdrop-blur-xl text-xs font-medium text-[var(--color-text-primary)] border border-[var(--color-glass-border)] hover:bg-[var(--color-text-primary)] hover:text-white transition-all duration-300 shadow-[var(--shadow-glass)]"
          >
            Quick Add
          </Link>
        </div>
      ...
      <div className="mt-5 flex-1 flex flex-col">
        ...
        <p className="text-xs text-[var(--color-text-tertiary)] mt-1.5 line-clamp-1">
          {product.tagline}
        </p>
        <div className="flex items-center gap-2.5 mt-3">
          <span className="text-sm font-serif font-medium text-[var(--color-text-primary)] tracking-tight">
        ...
        {product.colors && product.colors.length > 0 && (
          <div className="flex items-center gap-1.5 mt-3">
            {product.colors.slice(0, 5).map((color) => (
              <span
                key={color.name}
                className="w-[12px] h-[12px] rounded-full border border-[var(--color-border)] ring-[1.5px] ring-white"
