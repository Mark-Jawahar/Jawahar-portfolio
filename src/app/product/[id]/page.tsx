        ...
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-20">
              ...
              <div className="flex gap-3 mt-4">
                {product.images.map((img, i) => (
                  <button
                    ...
                    className={`relative w-20 h-24 rounded-[var(--radius-md)] overflow-hidden border-2 transition-all duration-300 ${
                      selectedImage === i
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              ...
              {product.isNew && (
                <span className="px-3 py-1 rounded-full bg-[var(--color-text-primary)] text-[10px] font-medium text-[var(--color-bg-primary)] tracking-wide">
                  New
                </span>
              )}
            </div>

            <h1 className="heading-md text-[var(--color-text-primary)]">
              {product.name}
            </h1>
            ...
            {product.colors && product.colors.length > 0 && (
              <div className="mt-8">
                <p className="text-xs font-medium text-[var(--color-text-secondary)] uppercase tracking-[0.15em] mb-3">
                ...
                <div className="flex flex-wrap gap-3">
                  {product.sizes.map((size) => (
                    <button
                      ...
                      className={`h-10 px-5 rounded-full text-xs font-medium border transition-all duration-200 ${
                        selectedSize === size
            ...
            <div className="mt-8">
              ...
              <div className="flex items-center border border-[var(--color-border)] rounded-full w-fit">
                <button
                  ...
                  className="w-11 h-11 flex items-center justify-center hover:bg-[var(--color-overlay)] rounded-full transition-colors"
                >
            ...
            <div className="flex gap-3 mt-10">
              <button
                ...
                className="flex-1 h-12 rounded-full bg-[var(--color-text-primary)] text-[var(--color-bg-primary)] text-sm font-medium hover:opacity-90 transition-all duration-300 active:scale-[0.97]"
              >
                Buy Now
              </button>
              <button
                onClick={handleAddToCart}
                className="flex-1 h-12 rounded-full border border-[var(--color-border)] text-[var(--color-text-primary)] text-sm font-medium hover:bg-[var(--color-overlay)] transition-all duration-300 active:scale-[0.97]"
              >
                {addedToCart ? "Added!" : "Add to Cart"}
              </button>
              <button
                className="w-12 h-12 rounded-full border border-[var(--color-border)] flex items-center justify-center hover:bg-[var(--color-overlay)] transition-all duration-300 flex-shrink-0"
            ...
            <div className="mt-8 p-6 rounded-[var(--radius-lg)] bg-[var(--color-bg-secondary)] border border-[var(--color-border)]">
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-3 text-sm">
                  ...
                  <span className="text-[var(--color-text-secondary)]">100% Handmade Guarantee</span>
                </div>
                {product.isPremiumPackaging && (
                  <div className="flex items-center gap-3 text-sm">
                    <HiOutlineGift className="w-4 h-4 text-[var(--color-accent)]" />
                    <span className="text-[var(--color-text-secondary)]">Premium Packaging</span>
                  </div>
                )}
                {product.isCustomizable && (
                  <div className="flex items-center gap-3 text-sm">
                    <HiOutlineSwatch className="w-4 h-4 text-[var(--color-accent)]" />
                    <span className="text-[var(--color-text-secondary)]">Customization Available</span>
                  </div>
                )}
              </div>
            </div>
              ...
              <div className="mt-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {product.occasions.map((occ) => (
                    <span
                      key={occ}
                      className="px-4 py-1.5 rounded-full bg-[var(--color-accent)]/10 text-[var(--color-accent)] text-xs"
                    >
                      {occ}
                    </span>
                  ))}
